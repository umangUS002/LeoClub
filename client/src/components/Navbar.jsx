  ...
  return (
    ...
    className={'flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-24 py-3.5 text-gray-600 relative transition-all sticky bg-primary top-0 z-100'}>
        ...
        <Link className='flex gap-2 items-center text-text1' to='/'>
            ...
            <motion.p whileHover={{scale: 1.05}} className='font-bold text-2xl'>LEO</motion.p>
        </Link>

        <div className={`text-text1 max-sm:fixed max-sm:h-screen text-sm max-sm:w-full max-sm:top-16 max-sm:border-t border-borderColor right-0 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 max-sm:p-4
        transition-all duration-300 z-50 bg-primary ${open ? 'max-sm:translate-x-0' : 'max-sm:translate-x-full'}`}>
            ...
            <button className='sm:hidden cursor-pointer bg-white' aria-label='Menu' onClick={()=>setOpen(!open)}>
