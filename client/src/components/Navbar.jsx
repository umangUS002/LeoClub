...
function Navbar() {
  ...
  return (
    ...
    className={'flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-24 py-3.5 text-gray-600 relative transition-all sticky bg-primary top-0 z-100'}>
        ...
        <div className={`text-text1 max-sm:fixed  max-sm:h-80vh backdrop-blur-sm text-sm max-sm:w-full max-sm:top-15 max-sm:border-t border-text1 right-0 flex flex-col sm:flex-row items-center sm:items-center gap-4 sm:gap-8 max-sm:p-4
        transition-all duration-300 z-50 bg-primary/50 ${open ? 'max-sm:translate-x-0' : 'max-sm:translate-x-full'}`}>
